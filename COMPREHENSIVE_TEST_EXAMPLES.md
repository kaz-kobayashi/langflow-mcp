...
このドキュメントは、langflow-mcpプロジェクトの全37個のMCPツールを網羅的にテストするための入力例を提供します。
...
12. [SCRM途絶リスク対応在庫最適化](#12-scrm途絶リスク対応在庫最適化-3ツール)
...
このドキュメントは、全37個のMCPツールを網羅的にテストするための入力例を提供しています。
...
## 12. SCRM途絶リスク対応在庫最適化（3ツール）

### 12.1 optimize_scrm_inventory_expected

**目的**: 期待値最小化による在庫最適化（途絶シナリオ対応）

**入力例**:
```
サプライチェーンの途絶リスクを考慮した在庫最適化を実行してください。
期待値最小化により、在庫保管費用と品切れ費用の期待値を最小化します。

データファイル: test_01（3工場、6つの小売拠点-製品組合せ）
...
途絶パラメータ:
- 途絶確率（disruption_prob）: {"0": 0.1, "1": 0.1, "2": 0.05}
- 回復時間（TTR）: {"0": 2, "1": 3, "2": 2}
- 最大同時途絶数（K_max）: 2
```
...
- 在庫保管費用: 約746.00
- 品切れ費用: 約746.00
- 最適在庫量: 各拠点-製品組合せの在庫レベル
- シナリオ数: 7（0途絶、単一途絶×3、2工場同時途絶×3）

---

### 12.2 optimize_scrm_inventory_cvar

**目的**: CVaR最小化による在庫最適化（リスク回避型）

**入力例**:
```
サプライチェーンの途絶リスクを考慮した在庫最適化を実行してください。
CVaR（Conditional Value at Risk）最小化により、リスク回避的な最適化を行います。
...
- 途絶確率（disruption_prob）: {"0": 0.1, "1": 0.1, "2": 0.05}
- 回復時間（TTR）: {"0": 2, "1": 3, "2": 2}
- 信頼水準（beta）: 0.95（95%信頼水準）
- 最大同時途絶数（K_max）: 2
```

**期待される結果**:
- CVaR: 約1492.00（95%信頼水準での条件付きVaR）
- VaR: 約1492.00（95%分位点のコスト）
- 期待コスト: 約1492.00
- 最適在庫量: 各拠点-製品組合せの在庫レベル
- シナリオ数: 7

**アルゴリズム詳細**:
- 目的関数: minimize θ + (1/(1-β)) * Σ(π_s * z_s)
  - θ: VaR（Value at Risk）
  - z_s: シナリオsにおける超過費用
  - π_s: シナリオsの発生確率
- 制約: z_s ≥ (総費用_s - θ) for all s
- betaが高いほど保守的（0.99 > 0.95 > 0.90）

---

### 12.3 compare_scrm_policies

**目的**: 期待値最小化とCVaR最小化の在庫方針を比較

**入力例**:
```
期待値最小化とCVaR最小化の2つの在庫方針を比較し、推奨方針を提示してください。
...
- 推奨: "期待値方針とCVaR方針のコストが同じです。CVaR方針を推奨します（リスク回避効果があります）。"

**推奨ロジック**:
- コスト増加率 < 5%: CVaR方針を推奨（リスク回避効果が高く、コスト増加が小さい）
- コスト増加率 5-15%: 状況に応じて選択（トレードオフを考慮）
- コスト増加率 > 15%: 期待値方針を推奨（CVaR方針のコスト増加が大きすぎる）

**適用シナリオ**:
1. サプライチェーン途絶リスクがある場合
2. 工場・倉庫の途絶確率が推定できる場合
3. 回復時間（TTR）が既知の場合
4. リスク許容度に応じた在庫方針を選択したい場合
5. 経営層への意思決定支援資料作成
